...
func TestGUIServerProfileSwitchingAndLockIsolation(t *testing.T) {
	...
	s1 := &store.EncryptedStore{Secrets: map[string]store.SecretEntry{
		"KEY_P1": {Value: "val1", Version: 1},
	}}
	s2 := &store.EncryptedStore{Secrets: map[string]store.SecretEntry{
	...
	mux.HandleFunc("/api/secrets", func(w http.ResponseWriter, r *http.Request) {
		...
		for k, v := range resp.Secrets {
			list = append(list, SecretItem{Key: k, Value: v.Value})
		}
		w.Header().Set("Content-Type", "application/json")
		_ = json.NewEncoder(w).Encode(map[string]interface{}{
			"profile": p,
	...
	var statusData2 map[string]interface{}
	_ = json.NewDecoder(resp2.Body).Decode(&statusData2)
	resp2.Body.Close()

	if statusData2["unlocked"] == true {
