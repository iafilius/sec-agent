...
func TestFormatReason(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			action:   "open",
			profile:  "default",
			expected: "sec-agent v2.14.1: Unlock vault for profile 'default'",
		},
		{
			...
			action:   "unlock",
			profile:  "production",
			expected: "sec-agent v2.14.1: Unlock vault for profile 'production'",
		},
		{
			name:     "switch profile",
			version:  "v2.14.1",
			action:   "switch",
			profile:  "work",
			expected: "sec-agent v2.14.1: Switch to profile 'work'",
		},
		{
			name:     "gui unlock",
			version:  "v2.14.1",
			action:   "gui",
			profile:  "default",
			expected: "sec-agent v2.14.1: Unlock vault for Web GUI (profile: 'default')",
		},
		{
			name:     "create profile",
			version:  "v2.14.1",
			...
			profile:  "dev",
			expected: "sec-agent v2.14.1: Authorize creation of profile 'dev'",
		},
		{
			name:     "migrate v2",
			version:  "v2.14.1",
			action:   "migrate_v2",
			profile:  "",
			expected: "sec-agent v2.14.1: Authorize v2.0 vault migration",
		},
		{
