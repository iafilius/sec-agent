...
func TestFormatReason(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			action:   "open",
			profile:  "default",
			expected: "sec-agent v2.13.2: Unlock vault for profile 'default'",
		},
		{
			...
			action:   "unlock",
			profile:  "production",
			expected: "sec-agent v2.13.2: Unlock vault for profile 'production'",
		},
		{
			name:     "switch profile",
			version:  "v2.13.2",
			action:   "switch",
			profile:  "work",
			expected: "sec-agent v2.13.2: Switch to profile 'work'",
		},
		{
			name:     "gui unlock",
			version:  "v2.13.2",
			action:   "gui",
			profile:  "default",
			expected: "sec-agent v2.13.2: Unlock vault for Web GUI (profile: 'default')",
		},
		{
			name:     "create profile",
			version:  "v2.13.2",
			...
			profile:  "dev",
			expected: "sec-agent v2.13.2: Authorize creation of profile 'dev'",
		},
		{
			name:     "migrate v2",
			version:  "v2.13.2",
			action:   "migrate_v2",
			profile:  "",
			expected: "sec-agent v2.13.2: Authorize v2.0 vault migration",
		},
		{
