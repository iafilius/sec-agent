...
# sec-agent Secrets Management Integration (v2.13.0)
...
1. **Dynamic Version Drift Detection**: When executing `sec status`, `sec status --quick`, or any CLI command, if the CLI output indicates:
   * A structured upgrade notice (`[sec-agent] ⚡ Upgraded AI agent skill from ... to v2.13.0`), or
   * A CLI version newer than this document's declared version (`v2.13.0`),
...
`[sec-agent] Automatically upgraded AI agent skills (v2.13.0) across N location(s).`
